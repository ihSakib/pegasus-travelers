...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    $checkUserStmt = $pdo->prepare("SELECT COUNT(*) FROM users WHERE email = :email");
    $checkUserStmt->execute(['email' => $email]);
    $isRegistered = $checkUserStmt->fetchColumn() > 0 ? 'yes' : 'no';

    // Insert data into the contact_messages table
    $insertStmt = $pdo->prepare("
            INSERT INTO contact_messages (first_name, last_name, phone, email, subject, message, registered)
            VALUES (:first_name, :last_name, :phone, :email, :subject, :message, :registered)
        ");
    $insertStmt->execute([
    ...
    // Send the email
    sendEmail($firstName, $lastName, $phone, $email, $subject, $message, $isRegistered);

    // Set success message and redirect
    $_SESSION['sent_success'] = 'Message sent successfully';
    ...
    $pdo->rollBack();
    echo "Failed to submit message: " . $e->getMessage();
  }
}

function sendEmail($firstName, $lastName, $phone, $email, $subject, $message, $isRegistered)
{
  ...
  try {
   ...
   include 'smtp/config.php';
    ...
    $mail->addAddress('ihsakib@outlook.com');
    ...
    $mailBody = "
            ...
            <head>
                <style>
                    .container { font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4; border-radius: 8px; }
                    .header { text-align: center; font-size: 24px; color: #333; }
                    .content { margin-top: 20px; padding: 20px; background-color: #fff; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); }
            ...
            <body>
                <div class='container'>
                    ...
                    <div class='content'>
                        ...
                        <p><strong>Registered:</strong> $isRegistered</p>
                    </div>
                    <div class='footer'> &copy; 2024 Pegasus Travelers. All rights reserved.</div>
                </div>
            </body>
            </html>
        ";
...
<html lang="en">
  ...
  <head>
    ...
    <title>Contact Us - Pegasus Travelers</title>
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
  </head>

  <body>
    <?php include "client/components/header.php" ?>
    <main class="bg-[url('img/contact-hero.jpg')] bg-cover  bg-bottom min-h-dvh">
      <!--******* contact us section************ -->
      <section class="pt-[100px] pb-20">
        <div class="w-[90%] max-lg:max-w-3xl mx-auto  my-6 font-[sans-serif]">
          <div class="text-center  px-6">
            <h2 class="text-gray-800 text-2xl   lg:text-4xl font-extrabold">Contact Us</h2>
            <p class="text-xs md:text-sm  text-gray-500 mt-4">Have some big idea or brand to develop and need help?</p>
          </div>
          <p class="text-center -mb-6 text-green-600">
          ...
          <div style="background-color: rgba(255, 255, 255, .9);"
            class="grid lg:grid-cols-3 items-start gap-4 p-2 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-lg mt-10">

            <div class="bg-[#011c2b] rounded-md p-6 h-full ">
              ...
              <ul class="mt-6 space-y-8">
                <li class="flex items-center">
                  ...
                  <a href="mailto:pegasustravelersbd@gmail.com" action="_blank"
                    class=" text-sm text-gray-500 hover:text-gray-400 ml-4">
                    pegasustravelersbd@gmail.com
                  </a>
                </li>
                <li class="flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" fill='#fff'
                  ...
                  <a href="tel:+8801864946718" class=" text-sm text-gray-500 hover:text-gray-400 ml-4">
                    +8801864946718
                  </a>
                </li>
                <li class="flex items-center">
                  ...
                  <a href="https://maps.app.goo.gl/5UeUB5CXdZWDgYD18" action="_blank"
                    class=" text-sm text-gray-500 hover:text-gray-400 ml-4">
                    Flat: 3/A, TA 199,Baluchar Road, South Badda, Dhaka 1212
                  </a>
                </li>
              </ul>
              ...
              <ul class="flex flex-wrap gap-4 mt-6">
                <li
                  ...
                  <a href="https://www.facebook.com/pegasustravelers/" target="_blank">
                ...
                <li
                  class="bg-gray-800 hover:bg-gray-900 h-10 w-10 rounded-full flex items-center justify-center shrink-0">
                  <a href="https://www.instagram.com/pegasustravelers" target="_blank">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" fill='#fff' viewBox="0 0 24 24">
                      <path
                ...
                <div class="grid sm:grid-cols-2 gap-8">
                  ...
                  <div class="relative flex items-center sm:col-span-2">
                    <textarea placeholder="Write Message" name="message"
                      class="px-2 pt-3 bg-white  w-full text-sm text-gray-800 border-b border-gray-300 focus:border-blue-500 outline-none"
                      required></textarea>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="#bbb" stroke="#bbb"
                      class="w-[18px] h-[18px] absolute right-2" viewBox="0 0 682.667 682.667">
                      <defs>
                  ...
                  <div class="col-span-full">
                    ...
                    <div class="flex max-lg:flex-col gap-6 mt-4">
                      <div class="flex items-center">
                        <input id="radio1" type="radio" name="subject" class="hidden peer" checked
                      ...
                      <div class="flex items-center">
                        <input id="radio2" type="radio" name="subject" class="hidden peer" value="Technical Support" />
                        <label for="radio2"
                          class="relative p-0.5 flex items-center justify-center shrink-0 peer-checked:before:hidden before:block before:absolute before:w-full before:h-full before:bg-white w-5 h-5 cursor-pointer border-2 border-[#011c2b] rounded-full overflow-hidden">
                          <span class="border-[4px] border-[#011c2b] rounded-full w-full h-full"></span>
                        </label>
                      ...
                      <div class="flex items-center">
                        <input id="radio3" type="radio" name="subject" class="hidden peer" value="Website Feedback" />
                        <label for="radio3"
                          class="relative p-0.5 flex items-center justify-center shrink-0 peer-checked:before:hidden before:block before:absolute before:w-full before:h-full before:bg-white w-5 h-5 cursor-pointer border-2 border-[#011c2b] rounded-full overflow-hidden">
                          <span class="border-[4px] border-[#011c2b] rounded-full w-full h-full"></span>
                        </label>
